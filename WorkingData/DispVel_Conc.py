...
for x in run:
    ...
    for y in arch:
        ...
        if 'Subhalo' and 'Group' in file_data:
            ...
            got_data = file_data['Subhalo'][dataParam][:] * 1e0  # type: ignore


            # Buscando Ajuste
            k, loc, scale = scp.fit(got_data,)
            mean_cal, std_cal = scp.mean(k, loc,scale), scp.std(k, loc,scale) # Calculo de la media y std
...
fig1.tight_layout()
fig1.savefig('Documento/images/Conc/VelDisp_Mean_Conc.png')
fig2.tight_layout()
fig2.savefig('Documento/images/Conc/VelDisp_Std_Conc.png')
plt.show()